...
rotate.somites<-function(somiteDF,bros,side){
  ...
  for(n in 1:bros){
    ...
    #shift somites by somite 8-9 boundary so that 8-9 boundary is at (0,0)
    ...
    a1$Xr=(a1$Xt*cos(ang)-a1$Yt*sin(ang)) # rotate around origin (som 8-9 bound)
    # if somite 5 is located before somite 1, this will transform the data so that it is now the opposite
    if(c$X<b$X){
      a1$Xr<-(-1*a1$Xr)
    }
    a1$Xr<-a1$Xr #shift som 8-9 bound
