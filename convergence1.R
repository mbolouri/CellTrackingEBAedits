...
#' @param Asom Anterior somite group that you want to measure convergence for. Default value is 9
#' @param Psom Posterior somite group that you want to measure convergence for. Dfault value is 18
#' @export


convergence<-function(dataframe, Asom=9,Psom=18){
  ...
  a<-as.data.frame(a)
  #value somite 1
  s9<-filter(a,Somite==Asom)
  s18<-filter(a,Somite==Psom)
  f<-s4%>%
