...
rotate<-function(drift.correctedDF, somiteDF, bros,side){
  ...
  for(n in 1:bros){
    ...
    b<-dplyr::filter(somiteDF,Embryo==n, boundary=="8-9")
    c<-dplyr::filter(somiteDF,Embryo==n,boundary=="18-19")

    #first translate so that the point of rotation (boundary 8-9) is at the origin
    a<-mutate(a,
              Xt=Xd-b$X,
              Yt=Yd-b$Y)
    ...
    #rotate around somite 8-9 boundary
    a$Xr=(a$Xt*cos(ang)-a$Yt*sin(ang)) # rotate around origin (som 8-9 bound)
    # if somite 5 is located before somite 1, this will transform the data so that it is now the opposite
    if(c$X<b$X){
      a$Xr<-(-1*a$Xr)
    }
    a$Xr<-a$Xr #shift som 8-9 bound
